...
select C.VAT, C._name, PN.phone
from  client as C, phone_number_client as PN, (appointment as A natural right outer join consultation as Cons)
where C.VAT = A.VAT_client and Cons.VAT_doctor = all(select E.VAT
                                                  from (appointment as A) natural join (employee as E)
                                                  where E._name = 'Jane Sweettooth')
order by C._name;
